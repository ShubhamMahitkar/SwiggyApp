...
def save_state(request):
    ...
    if sf.is_valid():
        ...
        return render(request, 's_admin/open_state.html', {"sf":StateForm()})
...
def update_state_data(request):
    ...
    StateModel.objects.filter(state_no=sno, state_name=sname).update(state_name=sname)
    return redirect('open_state')
...
def save_city(request):
    ...
    if cf.is_valid():
        ...
        return render(request, 's_admin/open_city.html', {"sf": CityForm()})
...
def update_city_data(request):
    sno = request.POST.get("s1")
    sname = request.POST.get("s2")
    CityModel.objects.filter(city_no=sno, city_name=sname).update(city_name=sname)
    return redirect('open_city')
...
def save_area(request):
    cf = AreaForm(request.POST)
    if cf.is_valid():
        cf.save()
        return redirect('open_city')
    else:
        return render(request, 's_admin/open_area.html', {"sf": AreaForm()})
...
def update_area_data(request):
    sno = request.POST.get("s1")
    sname = request.POST.get("s2")
    AreaModel.objects.filter(area_no=sno, area_name=sname).update(area_name=sname)
...
def update_type_data(request):
    ...
    RestaurantTypeModel.objects.filter(sno=sno, sname=sname).update(sname=sname)
    return redirect('open_type')
